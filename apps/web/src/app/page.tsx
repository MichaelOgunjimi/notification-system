    ...
    >
      <div className="relative z-30 border-b border-[var(--site-line)] bg-[var(--site-elevated)] px-4 py-2.5 text-center text-[11px] tracking-[0.01em] text-[var(--site-muted)]">
        ...
        <span className="hidden sm:inline">
          Notification infrastructure for teams that need the full delivery story.
        </span>
        <Link
      ...
      <header className="sticky top-3 z-40 px-3 sm:px-4">
        <nav className="site-nav mx-auto flex h-16 w-full max-w-[1240px] items-center gap-8 px-3 sm:px-4">
          ...
          >
            <BrandLogo
              priority
              className="transition-transform duration-300 ease-[var(--site-ease)] group-hover:-translate-y-0.5"
            />
          </Link>
      ...
      <div className="site-page-grid mx-auto mt-7 w-[calc(100%-1rem)] max-w-[1240px] sm:mt-8 sm:w-[calc(100%-2rem)]">
          ...
          <div className="grid overflow-hidden bg-[var(--site-panel)] lg:min-h-[570px] lg:grid-cols-[minmax(0,1.12fr)_minmax(390px,0.88fr)]">
            <div className="relative isolate flex flex-col justify-center overflow-hidden px-6 py-11 sm:px-10 sm:py-14 lg:px-14 lg:py-14 xl:px-[68px]">
              ...
              <AnimateOnScroll variant="fade-up" className="relative z-10">
                ...
                <h1 className="max-w-[620px] text-[clamp(2.15rem,10.4vw,4.65rem)] font-medium leading-[0.94] tracking-[-0.065em]">
                  ...
                  <span className="block text-[var(--site-muted-bright)] sm:whitespace-nowrap">
                    Under control.
                  </span>
                </h1>
              </AnimateOnScroll>
              ...
              <AnimateOnScroll variant="fade-up" delay={90} className="relative z-10">
                <p className="mt-6 max-w-[510px] text-[15px] leading-7 text-[var(--site-muted)] text-pretty sm:text-[16px]">
                  One API for email, SMS, and webhooks, with queues, retries, templates, and a
              ...
              <AnimateOnScroll variant="fade-up" delay={150} className="relative z-10">
                <div className="mt-7 grid grid-cols-2 gap-2 sm:max-w-[390px]">
                  <Link
                    href={docsUrl("/quickstart")}
                    className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--site-accent)]"
                  >
                    <ArrowButton label="Start building" className="w-full" />
                  </Link>
                  <Link
                    href={docsUrl()}
                    className="site-secondary-action inline-flex h-12 w-full items-center justify-center px-4 text-[13px] font-medium"
                  >
                    Explore the docs
                  </Link>
                </div>
            ...
            <div className="grid border-t border-[var(--site-line)] bg-[var(--site-elevated)] lg:grid-rows-[1fr_auto] lg:border-l lg:border-t-0">
              ...
              <div className="flex items-center justify-between gap-4 border-t border-[var(--site-line)] bg-[var(--site-panel)] px-5 py-4 sm:px-7 sm:py-5">
                <div>
                  <p className="text-[11px] font-semibold text-[var(--site-ink)]">
                    One event, three routes
                  </p>
                  <p className="mt-1 text-[10px] text-[var(--site-muted)]">
                    Each channel keeps its own delivery history.
                  </p>
                </div>
                <div className="flex items-center gap-1.5" aria-label="Supported channels">
                  {channels.map(({ icon: Icon, label }) => (
                    <span
                      key={label}
                      title={label}
                      className="grid size-8 place-items-center rounded-[8px] border border-[var(--site-line-strong)] bg-[var(--site-overlay)] text-[var(--site-ink)]"
                    >
        ...
        <section className="site-grid-section" aria-label="Platform guarantees">
          <div className="grid w-full grid-cols-2 md:grid-cols-12">
            {platformGuarantees.map(({ value, label, detail }, index) => (
              ...
              >
                <p className="font-mono text-[17px] font-medium tracking-[-0.04em] text-[var(--site-accent)] sm:text-[20px]">
                  {value}
                </p>
                <p className="mt-1 text-[11px] font-medium text-[var(--site-ink)] sm:text-[12px]">
                  {label}
                </p>
                <p className="mt-1.5 hidden text-[11px] text-[var(--site-muted)] sm:block">
                  {detail}
                </p>
              </div>
            ))}
          ...
          <div className="px-6 py-20 sm:px-10 sm:py-24 lg:px-14 lg:py-28">
            <AnimateOnScroll>
              <div className="max-w-[720px]">
                ...
                <p className="mt-5 max-w-[610px] text-[15px] leading-7 text-[var(--site-muted)]">
                  Your application emits business events. Beaco turns them into dependable customer
                  messages and keeps the operational detail attached.
                </p>
              </div>
            </AnimateOnScroll>
            ...
            <div className="mt-14 border border-[var(--site-line)]">
              <div className="grid overflow-hidden bg-[var(--site-elevated)] lg:grid-cols-12">
                <AnimateOnScroll
                  variant="fade-right"
                  className="min-w-0 p-6 sm:p-8 lg:col-span-7 lg:p-10 xl:p-12"
                >
                ...
                <StaggerGroup
                  className="border-t border-[var(--site-line)] bg-[var(--site-soft)] p-6 sm:p-8 lg:col-span-5 lg:border-l lg:border-t-0 lg:p-10"
                  staggerMs={70}
                >
                    ...
                    <StaggerItem key={step.title}>
                      <div className="grid grid-cols-[38px_1fr] gap-4 pb-8 last:pb-0">
                        ...
                        <div className="border-b border-[var(--site-line)] pb-8 last:border-b-0">
                          ...
                          <p className="mt-1.5 max-w-[360px] text-[12px] leading-5 text-[var(--site-muted)]">
                            {step.description}
                          </p>
                        </div>
                      </div>
        ...
        <section
          id="reliability"
          className="site-grid-section site-grid-field site-grid-field-reverse"
        >
          <div className="px-6 py-20 sm:px-10 sm:py-24 lg:px-14 lg:py-28">
            <AnimateOnScroll>
              <div className="max-w-[760px]">
                ...
                <p className="mt-5 max-w-[610px] text-[15px] leading-7 text-[var(--site-muted)]">
                  Queues and providers fail in different ways. Beaco keeps each recovery path
                  explicit, inspectable, and safe to operate.
                </p>
              </div>
            </AnimateOnScroll>

            <StaggerGroup
              className="mt-14 grid border-l border-t border-[var(--site-line)] lg:grid-cols-12"
              staggerMs={65}
            >
              {reliabilityFeatures.map(({ icon: Icon, title, description, className }, index) => (
                <StaggerItem key={title} className={className}>
                  <article
                    className={`site-feature-card group relative min-h-[250px] overflow-hidden border-b border-r border-[var(--site-line)] p-7 sm:p-9 ${index === 0 || index === 3 ? "bg-[var(--site-soft)] lg:min-h-[315px]" : "bg-[var(--site-panel)] lg:min-h-[275px]"}`}
                  >
                    ...
                    <div
                      className={`${index === 0 || index === 3 ? "mt-20" : "mt-14"} max-w-[520px]`}
                    >
