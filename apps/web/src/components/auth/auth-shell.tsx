        ...
        <header className="flex h-16 items-center justify-between border-b border-[var(--site-line)]">
          <Link href="/" aria-label="Beaco home">
            <BrandLogo
              ...
              labelClassName="text-[15px] font-semibold tracking-[-0.03em] text-[var(--site-ink)]"
            />
          </Link>
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/"
              className="inline-flex min-h-11 items-center gap-2 text-[13px] text-[var(--site-muted-bright)] transition-colors hover:text-[var(--site-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--site-accent)]"
            >
        ...
        <div className="grid flex-1 lg:grid-cols-[minmax(0,0.92fr)_minmax(32rem,1.08fr)]">
          <section className="auth-story relative hidden flex-col justify-between overflow-hidden border-r border-[var(--site-line)] px-10 py-16 lg:flex xl:px-16">
            <div className="relative z-10 max-w-xl">
              <p className="mb-6 font-mono text-[11px] tracking-[0.14em] text-[var(--site-accent)]">
                HUMAN ACCESS / PASSWORDLESS
              </p>
              <h1 className="max-w-[12ch] text-balance text-[clamp(2.6rem,5vw,5rem)] font-semibold leading-[0.95] tracking-[-0.065em]">
                {title}
              </h1>
              <p className="mt-7 max-w-[34rem] text-pretty text-[15px] leading-7 text-[var(--site-muted-bright)] sm:text-base">
                {description}
              </p>
            </div>
            ...
            <div className="relative z-10 mt-16 grid max-w-xl gap-6 sm:grid-cols-3 lg:mt-24 lg:grid-cols-1 xl:grid-cols-3">
              {[
                ...
                [
                  BellRinging,
                  "Project separation",
                  "Human sessions never become application API keys.",
                ],
                [
                  CheckCircle,
                  "Short sessions",
                  "Access tokens rotate through a durable refresh session.",
                ],
              ].map(([Icon, label, copy]) => {
                ...
                return (
                  <div
                    key={String(label)}
                    className="border-l border-[var(--site-line-strong)] pl-4"
                  >
                    <ItemIcon
                      size={18}
                      className="mb-4 text-[var(--site-accent)]"
                      aria-hidden="true"
                    />
