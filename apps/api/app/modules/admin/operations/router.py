"""Platform operations administration endpoints."""
...
from app.modules.delivery.processing.queues import channel_queue, dispatcher_queue
...
async def list_admin_keys(*, db: SessionDep, _: ApiKeysAdminDep) -> list[AdminKeyStats]:
...
async def get_admin_health(*, db: SessionDep, _: HealthAdminDep) -> AdminHealthResponse:
    database_ok = True
    try:
...
async def get_admin_analytics(*, db: SessionDep, _: AnalyticsAdminDep) -> AdminAnalyticsResponse:
...
async def list_admin_audit_log(
    ...
    *,
    ...
    """List all audit log entries for authorized platform administrators.
...
async def list_admin_usage(
    ...
    *,
    ...
    """List usage records across all keys for authorized platform administrators.
