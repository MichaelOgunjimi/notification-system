...
from app.modules.events.enums import EventPriority
from app.modules.events.schemas import EventCreate, RecipientCreate
from app.modules.events.service import _resolve_recipient_address
...
async def test_create_event_returns_is_duplicate_false_for_new_event() -> None:
    ...
    with (
        patch("app.modules.events.service.idempotency_service.check", return_value=None),
        patch(
            "app.modules.events.service.get_event_notification_ids",
...
async def test_create_event_returns_is_duplicate_true_for_existing_idempotency_key() -> None:
    ...
    with (
        patch(
            "app.modules.events.service.idempotency_service.check",
