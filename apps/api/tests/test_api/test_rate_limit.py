...
@pytest.mark.asyncio
async def test_rate_limit_regular_key_general(monkeypatch, auth_client: AsyncClient) -> None:
    # Keep all requests in one fixed window even when the test starts close to
    # a real minute boundary.
...
@pytest.mark.asyncio
async def test_non_api_key_requests_are_not_api_key_rate_limited(client: AsyncClient) -> None:
