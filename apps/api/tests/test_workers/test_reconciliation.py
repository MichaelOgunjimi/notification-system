...
from app.modules.delivery.processing.reconciliation import reconcile_stuck_notifications
...
def _seed_event_and_notification(
    ...
    *,
    ...
    now = utc_now()
    api_key = create_sync_project_api_key(session, name="reconciliation-test-key")
...
def test_sweep_batch_limit_respected(mock_get_session, mock_send_task):
    ...
    with patch("app.modules.delivery.processing.reconciliation.SWEEP_BATCH_LIMIT", batch_size):
        mock_get_session.return_value = _get_test_session()
        result = reconcile_stuck_notifications.apply().get()
