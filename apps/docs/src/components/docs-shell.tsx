  ...
  return (
    <DocsSearchContext.Provider value={() => setSearchOpen(true)}>
      <div className="docs-shell min-h-dvh bg-(--docs-canvas) text-(--docs-ink)">
        <header className="docs-topbar fixed inset-x-0 top-0 z-40 h-16 border-b border-[var(--docs-line)]">
          <div className="mx-auto grid h-full max-w-360 grid-cols-[1fr_auto] items-center gap-4 px-4 lg:grid-cols-[auto_minmax(280px,620px)_1fr] lg:px-8">
            <div className="flex min-w-0 items-center gap-3">
              <button
                ...
                className="grid size-9 place-items-center rounded-md border border-[var(--docs-line)] text-[var(--docs-muted)] lg:hidden"
              >
                <List size={19} />
              </button>
        ...
        {drawerOpen ? (
          <button
            ...
            className="fixed inset-0 z-40 bg-[var(--docs-backdrop)] backdrop-blur-sm lg:hidden"
            onClick={() => setDrawerOpen(false)}
          />
        ) : null}

        <aside
          className={`fixed bottom-0 left-0 top-0 z-50 w-71 border-r border-[var(--docs-line)] bg-[var(--docs-drawer)] pt-16 transition-transform duration-200 lg:hidden ${
